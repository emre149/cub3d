...
{
    if (map_info->side == 0)
        map_info->wall_x = map_info->pos[Y] + map_info->wall_dist * map_info->ray_dir[Y];
    else
        map_info->wall_x = map_info->pos[X] + map_info->wall_dist * map_info->ray_dir[X];
    map_info->wall_x -= floor(map_info->wall_x);
    map_info->tex[X] = (int)(map_info->wall_x * (double)IMG_SIZE);
    if (((map_info->side == 0) && (map_info->ray_dir[X] > 0)) || (map_info->side == 1 && map_info->ray_dir[Y] < 0))
        map_info->tex[X] = IMG_SIZE - map_info->tex[X] - 1;
    map_info->tex_step = 1.0 * IMG_SIZE / map_info->line_height;
    map_info->tex_pos = (map_info->draw_start - SCREEN_HEIGHT / 2 + map_info->line_height / 2) * map_info->tex_step;
    ...
    while (map_info->y < map_info->draw_end)
    {
        ...
        map_info->color = map_info->text[map_info->tex_num][IMG_SIZE * map_info->tex[Y] + map_info->tex[X]];
